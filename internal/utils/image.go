...
var backgroundCache map[string]image.Image = make(map[string]image.Image)
...
func loadBackground(fileName string) (image.Image, error) {
	if im, ok := backgroundCache[fileName]; ok {
		return im, nil
	}

	im, err := gg.LoadPNG(fileName)
	if err == nil {
		backgroundCache[fileName] = im
	}
...
func GenImage(fileName string, title string, sub1 string, sub2 string, backgroundPNG string) error {
	background, err := loadBackground(backgroundPNG)
	if err != nil {
		return err
	}
	...
	dc.DrawRectangle(0, d, float64(w), h1+d1) // h1+d2
	dc.Fill()
